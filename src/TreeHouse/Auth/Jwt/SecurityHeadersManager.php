...
/**
 * JWT Security Headers Manager
 *
 * Manages security headers for JWT authentication responses to enhance
 * application security and prevent common attacks. Provides configurable
 * security policies and automatic header injection.
 *
 * Features:
 * - Content Security Policy (CSP) management
 * - CORS headers for JWT APIs
...
class SecurityHeadersManager
{
     ...
     * @param array $context Additional context (JWT info, rate limits, etc.)
    ...
    public function applyHeaders(Response $response, ?Request $request = null, array $context = []): Response
    {
        ...
        $response = $this->applyJwtHeaders($response, $context);
     ...
     *
     * @param Response $response HTTP response
     * @param array $context JWT context information
     * @return Response Modified response
     */
    private function applyJwtHeaders(Response $response, array $context): Response
    {
        $jwtConfig = $this->config['jwt'];

        // Rate limiting headers
        if ($jwtConfig['include_rate_limit_headers'] && isset($context['rate_limit'])) {
            ...
            if (isset($rateLimit['reset'])) {
                $response->setHeader('X-RateLimit-Reset', (string) $rateLimit['reset']);
            }
        }

        // Token information headers (development only)
        if ($jwtConfig['expose_token_info'] && !$this->isProduction && isset($context['token_info'])) {
            $tokenInfo = $context['token_info'];
            if (isset($tokenInfo['expires_at'])) {
                $response->setHeader('X-JWT-Expires-At', (string) $tokenInfo['expires_at']);
            }
            if (isset($tokenInfo['issued_at'])) {
                $response->setHeader('X-JWT-Issued-At', (string) $tokenInfo['issued_at']);
            }
            if (isset($tokenInfo['key_id'])) {
                $response->setHeader('X-JWT-Key-ID', $tokenInfo['key_id']);
            }
        }
    ...
    {
        $customHeaders = $this->config['jwt']['custom_headers'];
    ...
    public function getHeadersSummary(): array
    {
        return [
            ...
            'custom_headers_count' => count($this->config['jwt']['custom_headers']),
